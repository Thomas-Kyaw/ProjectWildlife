...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allowing all origins (you can specify allowed origins for production)
...
def extract_date_time_temperature(text):
    ...
    # Regex patterns for date, time, and temperature
    ...
    temp_pattern = r'\b-?\d{1,2}\s?[°C°F]\b'  # Matches temperature format (e.g., 20°C or 20°F)
...
async def detect(file: UploadFile = File(...)):
    ...
    annotated_image_url = f"http://192.168.1.62:8000/static/annotated/{annotated_filename}"
    csv_url = f"http://192.168.1.62:8000/static/{output_csv_file.replace('output/', '')}"
